...
正如第\ref{sec6.5}节中所展示的，Nash (1951) 借助布劳威尔不动点定理（定理~\ref{theo:6.4}）证明了有限策略型博弈中混合策略纳什均衡的存在性。不动点定理是证明经济学中诸多均衡概念存在性的有力工具，而布劳威尔不动点定理是其中第一个也是最重要的一个。
...
\item Knaster, Kuratowski, and Mazurkiewicz
(1929)  通过对斯佩纳引理应用极限过程证明了KKM引理（定理~\ref{theo:7.12}），并由此推出 \(\Delta\) 上连续函数不动点的存在性。
\item 这个极限过程的非平凡部分是找到越来越精细的三角剖分，为此我们在本章末尾介绍“弗罗伊登塔尔”三角剖分。
\item 为了证明 \(C\) 上连续函数的不动点定理，将一般的紧凸集 \(C\) “嵌入”到单位单纯形中。
\end{itemize}

主要步骤是斯佩纳引理。Sperner
(1928) 在一个巧妙的归纳证明中，对完全标记子单纯形进行计数，以表明它们的数量是奇数，因此不为零。斯佩纳论证的核心基于一个核心的图论概念——无向图。
...
除了 Sperner (1928) 的奇偶性计数方法外，本章还给出了斯佩纳引理的一个简洁构造性证明。证明中的这个替代步骤以一种特殊方式将不同维度的图组合起来，使得从单纯形的一个顶点到一个完全标记子单纯形存在一条单一路径（见图~\ref{fig:7.10}）。定义该路径的子单纯形的维度可能会沿着路径多次上升和下降。
...
第\ref{sec-7.7}节详细处理了在任意维度下寻找单纯形越来越精细的三角剖分这一非平凡问题，该问题在证明中被放在后面。我们介绍了Freudenthal (1942) 提出的三角剖分方法。它有两个部分：首先，将单位立方体细分为小的“子立方体”，这在任何维度 \(d\) 下都适用；其次，将立方体和每个子立方体细分为 \(d!\)  （$d$ 的阶乘）个单纯形，这些单纯形是从立方体上所有可能的“单调路径”得到的。
...
不动点定理在Brouwer (1911)的论文结尾处被提出。在这篇以及相关论文中，布劳威尔发展并使用了连续函数的单纯逼近的“度”(一个点的原像数量)的概念。详细讨论见Brouwer (1976)。关于布劳威尔不动点定理及其后续扩展的广泛影响的历史综述见Park (1999)。

布劳威尔不动点定理是拓扑学中的一个重要定理。拓扑学的基本概念是连续性，但从更朴素的角度来看，拓扑学可以被视为对“位置”的定性研究。关于这一方向发表的第一篇论文，是 Euler (1741) 对“柯尼斯堡七桥”问题的解答（另见 Biggs, Lloyd, and Wilson, 1976）。欧拉的论文奠定了图论与拓扑学的基础；它引入了图的概念，并且节点的度在其中起到了核心作用。而斯佩纳引理与图中奇度节点必有偶数个这一性质相关，恰好与这一渊源形成了绝妙的契合。

定理~\ref{theo:7.10} 归功于Sperner (1928),，并且被Knaster, Kuratowski, and Mazurkiewicz (1929)用于借助他们的定理~\ref{theo:7.12} 证明单纯形上的不动点定理。正如我们在定理~\ref{theo:7.10} 的证明中所提到的，这些证明使用了方程 \eqref{eq:7.12} 中的模 2 计数。“奇偶性论证”这一术语由Papadimitriou (1994)创造。

路径跟踪的论证归功于Cohen(1967)。科恩的证明没有提供足够详细的内容，让人无法看到路径也可能回到低几个维度的情况，如图~\ref{fig:7.10}中的示例所示。更精确的描述请参阅Kuhn (1969)。我们将图的“内部”节点定义为 \({C}^{\prime }\) 中的子极面，而不是\eqref{eq:7.11}中 \(A\) 的元素。当处于固定维度时，更自然的做法是仅将单纯剖分中的子单纯形，即 \(D\) 和 \(A\) 的元素，视为图的节点，如果它们在 \({C}^{\prime }\) 中有一个公共子极面，则用一条边将它们连接起来。然而，特别是对于 \({G}_{2}\) 的构造，边更容易被看作线段，节点更容易被看作 \({\sum }_{2}\) 中的顶点。\eqref{eq:7.13}中 \(G\) 构造的一个关键论证是，图 \({G}_{n}\) 中某些路径的端点具有不同的维度，并且我们仅在端点处转换到更高的维度。

用于证明波尔查诺-魏尔施特拉斯定理的“海景酒店”的例子取自Bryant (1990)。

弗罗伊登塔尔单纯剖分归功于Freudenthal (1942)，并且被多次重新发现，例如Kuhn (1960)。
